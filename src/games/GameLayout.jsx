...
const GameLayout = ({ title, children, onPlaceBet, betDisabled }) => {
  const navigate = useNavigate();
  const { balance } = useWallet();

  return (
    <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex justify-center overflow-hidden">
      <div className="w-full max-w-md bg-casino-dark flex flex-col h-full relative shadow-2xl border-x border-gray-800/50">
        {/* Game Header */}
        <header className="h-14 bg-casino-card border-b border-gray-800 flex items-center justify-between px-4 shrink-0 shadow-lg relative z-20">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold tracking-tight text-white">{title}</span>
          </div>
        ...
        <main className="flex-1 relative overflow-y-auto pl-4 pr-3 pt-4 pb-12 custom-scrollbar scroll-smooth">
